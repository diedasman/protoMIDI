...
- Shield: `protomidi`
- Firmware path: `prototype/firmware/config/boards/shields/protomidi`
...
```sh
./prototype/firmware/build-local.sh nice_nano_v2 protomidi
```

The script keeps the ZMK checkout in `prototype/firmware/.zmk/workspace`, which
is ignored by git. The first run downloads the ZMK and Zephyr dependencies, so
it can take a while. Later runs reuse that workspace.
...
west build -s zmk/app -b nice_nano_v2 -- \
  -DSHIELD=protomidi \
  -DZMK_CONFIG=/absolute/path/to/protoMIDI/prototype/firmware/config
```
